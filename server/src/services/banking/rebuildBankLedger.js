...
export const rebuildBankLedger = async (organizationId) => {
  ...
  for (const raw of rawTransactions) {
    await upsertLedgerFromRaw(raw, organizationId);
  }

  console.log(`[BANK LEDGER] Rebuild completed`);
};

export const rebuildBankLedgerIncremental = async (
  organizationId,
  zohoTransactionIds = []
) => {
  const uniqueIds = [...new Set((zohoTransactionIds || []).filter(Boolean))];

  if (!uniqueIds.length) {
    console.log(`[BANK LEDGER] Incremental rebuild skipped for ${organizationId} (0 txns)`);
    return;
  }

  console.log(
    `[BANK LEDGER] Incremental rebuild started for ${organizationId} (${uniqueIds.length} txns)`
  );

  const rawTransactions = await RawZohoBankTransaction.find({
    organizationId,
    zohoTransactionId: { $in: uniqueIds },
  });
  ...
  console.log(
    `[BANK LEDGER] Incremental rebuild completed for ${organizationId} (${rawTransactions.length} found)`
  );
};